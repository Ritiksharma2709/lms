...
export const searchCourse = async (req, res) => {
  try {
    ...
    // 🔹 Convert categories to array and normalize to lowercase
    ...
    const searchCriteria = {
      ...
      $or: [
        ...
        { category: { $regex: query, $options: "i" } }, // handles query match on category
      ],
    };

    // 🔹 Add category filter (case-insensitive exact match)
    if (categories.length > 0) {
      searchCriteria.category = {
        $in: categories.map(
          (cat) => new RegExp(`^${cat}$`, "i") // match exactly but ignore case
        ),
      };
    }
...
export const getCreatorCourses=async (req,res)=>{
    try {
        ...
        return res.status(200).json({
         ...
         console.log(error);
        return res.status(500).json({
            message:"Failed o get the admin course"
        })
    }
}
...
if(!lectureTitle || !courseId){
    return res.status(400).json({
        message:"Lecture title is require"
    })
}
...
return res.status(201).json({
    lecture,
    message:"lecture created succeessfully"
});
